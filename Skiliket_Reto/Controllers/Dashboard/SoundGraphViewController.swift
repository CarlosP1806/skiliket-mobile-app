//
//  SoundGraphViewController.swift
//  Skiliket_Reto
//
...
class SoundGraphViewController: UIViewController {
    var selectedSensor: String?
    var selectedCity: String?

    @IBOutlet weak var graphContainerView: UIView!

    // Arreglo para almacenar las lecturas de sonido
    var soundData: [Sound] = []
    ...
    func loadGraph(sensor: String, city: String) {
        ...
        let graphView = UIHostingController(rootView: SoundLineChartUIView(soundData: soundData))
        addChild(graphView)
    ...
    func fetchData(sensor: String, city: String) {
        ...
        URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            do {
                ...
                let newSound = Sound(value: String(soundPT.sound), timeStamp: Date())
                
                // Agregar el nuevo sonido a los datos y actualizar la gráfica
                DispatchQueue.main.async {
                    self.soundData.append(newSound)
                    self.updateGraph()
                }
            } catch {
