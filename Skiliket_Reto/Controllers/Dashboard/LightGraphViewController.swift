//
//  LightGraphViewController.swift
//  Skiliket_Reto
//
...
class LightGraphViewController: UIViewController {
    ...
    func fetchData(sensor: String, city: String) {
        ...
        URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            do {
                let lightPT = try JSONDecoder().decode(LightPT.self, from: data)
                let newLight = Light(value: String(lightPT.light), timeStamp: Date())
                
                // Agregar el nuevo sonido a los datos y actualizar la gráfica
                DispatchQueue.main.async {
                    self.lightData.append(newLight)
                    self.updateGraph()
                }
            } catch {
